...
import t5750.pay.common.core.utils.WeixinConfigUtil;
import t5750.pay.controller.common.BaseController;
...
import t5750.pay.utils.JsonUtils;
...
public class ScanPayController extends BaseController {
	 ...
	 * 1:传入支付通道参数,跳转到相应的支付通道扫码页面 2:未传入支付通道参数,跳转到
	 * 
	 * @return
	 */
	...
	public String initPay(Model model) {
		...
		if (!MerchantApiUtil.isRightSign(paramMap,
				rpUserPayConfig.getPaySecret(), sign)) {
			LOG.info("===>订单签名异常");
		...
		if (StringUtil.isEmpty(payWayCode)) {// 非直连方式
			RpPayGateWayPageShowVo payGateWayPageShowVo = rpTradePaymentManagerService
					.initNonDirectScanPay(payKey, productName, orderNo,
							orderDate, orderTime, orderPrice, orderIp,
			...
			if (PayWayEnum.TEST_PAY.name().equals(payWayCode)) {
				...
				ScanPayResultVo scanPayResultVo = rpTradePaymentManagerService
						.initDirectScanPay(payKey, productName, orderNo,
								orderDate, orderTime, orderPrice, payWayCode,
	...
	public String toPay(@PathVariable("payKey") String payKey,
			@PathVariable("orderNo") String orderNo,
			@PathVariable("payWay") String payWay, Model model) {
		ScanPayResultVo scanPayResultVo = rpTradePaymentManagerService
				.toNonDirectScanPay(payKey, orderNo, payWay);
		model.addAttribute("codeUrl", scanPayResultVo.getCodeUrl());// 支付二维码
	...
	public void orderQuery(HttpServletResponse httpServletResponse)
			throws IOException {
		String payKey = getString_UrlDecode_UTF8("payKey"); // 企业支付KEY
	...
	private String toTestPay(Model model, String payKey, String productName,
