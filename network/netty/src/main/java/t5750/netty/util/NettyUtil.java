package t5750.netty.util;

/**
 *
 */
public class NettyUtil {
	public static final String INET_HOST = "127.0.0.1";
	public static final int PORT_8765 = 8765;
	public static final int PORT_8764 = 8764;
	public static final int FIXED_LENGTH_FRAME_DECODER = 5;
	// public static final String AUTH_IP = "192.168.36.2";
	public static final String AUTH_IP = "192.168.56.1";
	public static final String AUTH_KEY = "1234";
	/**
	 * Parent module name
	 */
	public static final String MODULE = "network";
	public static final String HTTPFILE_DEFAULT_URL = "/" + MODULE
			+ "/netty/src/main/resources/static/";
}
