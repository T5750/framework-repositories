package t5750.pay.service.trade;
...
public class PayServiceTrade {
	...
	private static void creatTrade(
			...
			RpTradePaymentRecordDao rpTradePaymentRecordDao)
		...
		for (int i = 0; i < 0; i++) {// 每条线程生成数
			...
			rpTradePaymentOrder.setOrderDate(sdf1.parse("2019-01-05"));// 下单日期
			...
			rpTradePaymentOrder.setReturnUrl("https://t5750.github.io");// 页面通知地址
			rpTradePaymentOrder.setNotifyUrl("https://t5750.github.io");// 后台通知地址
			...
			notifyMap.put("time_end", "20190105143625");
			...
			rpTransactionMessageService
					.saveAndSendMessage(rpTransactionMessage);
		}
	}
}