...
import t5750.pay.service.trade.biz.impl.RpTradePaymentManagerBizImpl;
...
import t5750.pay.service.trade.entity.weixinpay.WeiXinPrePay;
import t5750.pay.service.trade.enums.OrderFromEnum;
import t5750.pay.service.trade.enums.TradeStatusEnum;
import t5750.pay.service.trade.enums.alipay.AliPayTradeStateEnum;
import t5750.pay.service.trade.enums.weixinpay.WeiXinTradeTypeEnum;
import t5750.pay.service.trade.enums.weixinpay.WeixinTradeStateEnum;
import t5750.pay.service.trade.exceptions.TradeBizException;
import t5750.pay.service.trade.utils.MerchantApiUtil;
import t5750.pay.service.trade.utils.WeiXinPayUtils;
import t5750.pay.service.trade.utils.WeixinConfigUtil;
import t5750.pay.service.trade.utils.alipay.config.AlipayConfigUtil;
import t5750.pay.service.trade.utils.alipay.util.AlipayNotify;
import t5750.pay.service.trade.utils.alipay.util.AlipaySubmit;
...
/**
 * @功能说明: Pay订单管理服务接口实现,所有与接口相关,需要做数据修改,事务管理的类,由该接口管理
 * @版本:V1.0
 */
@Service("rpTradePaymentManagerService")
public class RpTradePaymentManagerServiceImpl implements
		RpTradePaymentManagerService {
	private static final Logger LOG = LoggerFactory
			.getLogger(RpTradePaymentManagerServiceImpl.class);
	@Autowired
	...
	public ScanPayResultVo initDirectScanPay(String payKey, String productName,
			String orderNo, Date orderDate, Date orderTime,
			BigDecimal orderPrice, String payWayCode, String orderIp,
			Integer orderPeriod, String returnUrl, String notifyUrl,
		...
		if (PayWayEnum.WEIXIN.name().equals(payWayCode)) {
			payWay = rpPayWayService.getByPayWayTypeCode(
					rpUserPayConfig.getProductCode(), payWayCode,
		...
		RpUserInfo rpUserInfo = rpUserInfoService
				.getDataByMerchentNo(merchantNo);
		if (rpUserInfo == null) {
			LOG.error("用户不存在");
		...
		if (rpTradePaymentOrder == null) {
			// 订单不存在则创建
			rpTradePaymentOrder = sealRpTradePaymentOrder(merchantNo,
					rpUserInfo.getUserName(), productName, orderNo, orderDate,
					orderTime, orderPrice, payWayCode,
					PayWayEnum.getEnum(payWayCode).getDesc(),
					rpUserPayConfig.getFundIntoType(), orderIp, orderPeriod,
			...
			if (rpTradePaymentOrder.getOrderAmount().compareTo(orderPrice) != 0) {
				throw new TradeBizException(
						TradeBizException.TRADE_ORDER_ERROR, "错误的订单");
			}
			if (TradeStatusEnum.SUCCESS.name().equals(
					rpTradePaymentOrder.getStatus())) {
				throw new TradeBizException(
						TradeBizException.TRADE_ORDER_ERROR, "订单已支付成功,无需重复支付");
			}
		}
		// 通过支付订单及商户费率生成支付记录
	...
	public void completeScanPay(String payWayCode, Map<String, String> notifyMap) {
		...
		LOG.info("------[接收到要处理的订单{}]--------[开始处理时间{}]------", bankOrderNo,
				DateUtils.formatDate(new Date(), "yyyy-MM-dd HH:mm:ss SSS"));
		// 根据银行订单号获取支付信息
		RpTradePaymentRecord rpTradePaymentRecord = rpTradePaymentRecordDao
				.getByBankOrderNo(bankOrderNo);
		...
		// 幂等判断
		if (TradeStatusEnum.SUCCESS.name().equals(
				rpTradePaymentRecord.getStatus())) {
			LOG.info("订单{}为成功状态,不做业务处理", bankOrderNo);
			return;
		}
		if (!TradeStatusEnum.WAITING_PAYMENT.name().equals(
				rpTradePaymentRecord.getStatus())) {
		...
		if (PayWayEnum.WEIXIN.name().equals(payWayCode)) {
			if (WeixinTradeStateEnum.SUCCESS.name().equals(
					notifyMap.get("result_code"))) {// 业务结果 成功
				String timeEndStr = notifyMap.get("time_end");
				if (!StringUtil.isEmpty(timeEndStr)) {
					timeEnd = DateUtils.getDateFromString(timeEndStr,
							"yyyyMMddHHmmss");// 订单支付完成时间
				}
				orderIsSuccess = true;
			...
			} else if (AliPayTradeStateEnum.TRADE_SUCCESS.name().equals(
					tradeStatus)) {
				String gmtPaymentStr = notifyMap.get("gmt_payment");// 付款时间
				if (!StringUtil.isEmpty(gmtPaymentStr)) {
					timeEnd = DateUtils.getDateFromString(gmtPaymentStr,
							"yyyy-MM-dd HH:mm:ss");
				}
				orderIsSuccess = true;
		...
		if (orderIsSuccess) {// 银行返回订单支付成功
			...
			rpTransactionMessageService
					.saveMessageWaitingConfirm(rpTransactionMessage);
			LOG.info("==>保存消息数据");
			try {
				// completeSuccessOrder(rpTradePaymentRecord, bankTrxNo,
				// timeEnd, bankReturnMsg);
				rpTradePaymentManagerBiz
						.completeSuccessOrder(rpTradePaymentRecord, bankTrxNo,
								timeEnd, bankReturnMsg);
			} catch (Throwable e) {
				...
				throw new TradeBizException(
						TradeBizException.TRADE_SYSTEM_ERROR, "交易系统异常,处理支付结果失败");
			}
			rpTransactionMessageService
					.confirmAndSendMessage(rpTransactionMessage.getMessageId());
			LOG.info("==>修改消息状态");
		} else {
			LOG.info("==>开始处理支付失败的订单结果");
			completeFailOrder(rpTradePaymentRecord);
		}
		LOG.info("------[结束要处理的订单{}]--------[结束处理时间{}]------", bankOrderNo,
				DateUtils.formatDate(new Date(), "yyyy-MM-dd HH:mm:ss SSS"));
	}
	 ...
	 */
	private void completeSuccessOrder(
		...
		if (FundInfoTypeEnum.PLAT_RECEIVES.name().equals(
			...
			LOG.info("==>修改订单账户金额");
			rpAccountTransactionService.creditToAccount(rpTradePaymentRecord
					.getMerchantNo(), rpTradePaymentRecord.getOrderAmount()
					.subtract(rpTradePaymentRecord.getPlatIncome()),
	 ...
	 * @return
	 */
	private String getMerchantNotifyUrl(
			RpTradePaymentRecord rpTradePaymentRecord,
			RpTradePaymentOrder rpTradePaymentOrder, String sourceUrl,
			TradeStatusEnum tradeStatusEnum) {
		RpUserPayConfig rpUserPayConfig = rpUserPayConfigService
				.getByUserNo(rpTradePaymentRecord.getMerchantNo());
		if (rpUserPayConfig == null) {
			throw new UserBizException(UserBizException.USER_PAY_CONFIG_ERRPR,
					"用户支付配置有误");
		}
		...
		String orderDateStr = DateUtils.formatDate(
				rpTradePaymentOrder.getOrderDate(), "yyyyMMdd"); // 订单日期
		paramMap.put("orderDate", orderDateStr);
		String orderTimeStr = DateUtils.formatDate(
				rpTradePaymentOrder.getOrderTime(), "yyyyMMddHHmmss"); // 订单时间
	...
	@Override
	@Transactional(rollbackFor = Exception.class)
	public RpPayGateWayPageShowVo initNonDirectScanPay(String payKey,
			String productName, String orderNo, Date orderDate, Date orderTime,
			BigDecimal orderPrice, String orderIp, Integer orderPeriod,
			String returnUrl, String notifyUrl, String remark, String field1,
		...
		List<RpPayWay> payWayList = rpPayWayService
				.listByProductCode(rpUserPayConfig.getProductCode());
		...
		if (rpTradePaymentOrder == null) {
			rpTradePaymentOrder = sealRpTradePaymentOrder(merchantNo,
					rpUserInfo.getUserName(), productName, orderNo, orderDate,
					orderTime, orderPrice, null, null,
					rpUserPayConfig.getFundIntoType(), orderIp, orderPeriod,
					returnUrl, notifyUrl, remark, field1, field2, field3,
					field4, field5);
			rpTradePaymentOrderDao.insert(rpTradePaymentOrder);
		...
		payGateWayPageShowVo.setProductName(rpTradePaymentOrder
				.getProductName());// 产品名称
		payGateWayPageShowVo.setMerchantName(rpTradePaymentOrder
				.getMerchantName());// 商户名称
		payGateWayPageShowVo.setOrderAmount(rpTradePaymentOrder
				.getOrderAmount());// 订单金额
		payGateWayPageShowVo.setMerchantOrderNo(rpTradePaymentOrder
				.getMerchantOrderNo());// 商户订单号
		...
		for (RpPayWay payWay : payWayList) {
			payWayEnumMap.put(payWay.getPayWayCode(),
	 ...
	 */
	@Override
	public ScanPayResultVo toNonDirectScanPay(String payKey, String orderNo,
			String payWayCode) {
		RpUserPayConfig rpUserPayConfig = rpUserPayConfigService
				.getByPayKey(payKey);
		if (rpUserPayConfig == null) {
	 ...
	 * @return
	 */
	private ScanPayResultVo getScanPayResultVo(
			RpTradePaymentOrder rpTradePaymentOrder, RpPayWay payWay) {
		...
		if (PayWayEnum.WEIXIN.name().equals(payWay.getPayWayCode())) {
			...
			rpTradePaymentOrder
					.setPayTypeName(PayTypeEnum.DIRECT_PAY.getDesc());// 支付方式
		}
		// 更新支付订单（因为支付方式可能会变换）
		...
		RpTradePaymentRecord rpTradePaymentRecord = sealRpTradePaymentRecord(
				rpTradePaymentOrder.getMerchantNo(),
				rpTradePaymentOrder.getMerchantName(),
				rpTradePaymentOrder.getProductName(),
				rpTradePaymentOrder.getMerchantOrderNo(),
				rpTradePaymentOrder.getOrderAmount(), payWay.getPayWayCode(),
				payWay.getPayWayName(), rpTradePaymentOrder.getFundIntoType(),
				BigDecimal.valueOf(payWay.getPayRate()),
				rpTradePaymentOrder.getOrderIp(),
				rpTradePaymentOrder.getReturnUrl(),
				rpTradePaymentOrder.getNotifyUrl(),
				rpTradePaymentOrder.getRemark(),
				rpTradePaymentOrder.getField1(),
				rpTradePaymentOrder.getField2(),
				rpTradePaymentOrder.getField3(),
				rpTradePaymentOrder.getField4(),
				rpTradePaymentOrder.getField5());
		rpTradePaymentRecordDao.insert(rpTradePaymentRecord);
		if (PayWayEnum.WEIXIN.name().equals(payWayCode)) {// 微信支付
			String appid = "";
			String mch_id = "";
			String partnerKey = "";
			if (FundInfoTypeEnum.MERCHANT_RECEIVES.name().equals(
					rpTradePaymentOrder.getFundIntoType())) {// 商户收款
				// 根据资金流向获取配置信息
				RpUserPayInfo rpUserPayInfo = rpUserPayInfoService.getByUserNo(
						rpTradePaymentOrder.getMerchantNo(), payWayCode);
				appid = rpUserPayInfo.getAppId();
				mch_id = rpUserPayInfo.getMerchantId();
				partnerKey = rpUserPayInfo.getPartnerKey();
			} else if (FundInfoTypeEnum.PLAT_RECEIVES.name().equals(
					rpTradePaymentOrder.getFundIntoType())) {// 平台收款
			...
			WeiXinPrePay weiXinPrePay = sealWeixinPerPay(appid, mch_id,
					...
					rpTradePaymentRecord.getBankOrderNo(), "",
					rpTradePaymentOrder.getOrderIp());
			String prePayXml = WeiXinPayUtils.getPrePayXml(weiXinPrePay,
					WeixinConfigUtil.readConfig("partnerKey"));
			// 调用微信支付的功能,获取微信支付code_url
			Map<String, Object> prePayRequest = WeiXinPayUtils.httpXmlRequest(
					WeixinConfigUtil.readConfig("prepay_url"), "POST",
					prePayXml);
			if (WeixinTradeStateEnum.SUCCESS.name().equals(
					prePayRequest.get("return_code"))
					&& WeixinTradeStateEnum.SUCCESS.name().equals(
							prePayRequest.get("result_code"))) {
				String weiXinPrePaySign = WeiXinPayUtils.geWeiXintPrePaySign(
						appid, mch_id, weiXinPrePay.getDeviceInfo(),
						WeiXinTradeTypeEnum.NATIVE.name(), prePayRequest,
						partnerKey);
				...
				if (prePayRequest.get("sign").equals(weiXinPrePaySign)) {
					rpTradePaymentRecord.setBankReturnMsg(prePayRequest
							.toString());
					rpTradePaymentRecordDao.update(rpTradePaymentRecord);
					...
					throw new TradeBizException(
							TradeBizException.TRADE_WEIXIN_ERROR, "微信返回结果签名异常");
				}
			} else {
				throw new TradeBizException(
						TradeBizException.TRADE_WEIXIN_ERROR, "请求微信异常");
			}
			...
			sParaTemp.put("anti_phishing_key",
					AlipayConfigUtil.anti_phishing_key);
			sParaTemp.put("exter_invoke_ip", AlipayConfigUtil.exter_invoke_ip);
			sParaTemp
					.put("out_trade_no", rpTradePaymentRecord.getBankOrderNo());
			sParaTemp.put("subject", rpTradePaymentOrder.getProductName());
			sParaTemp.put("total_fee", String.valueOf(rpTradePaymentOrder
					.getOrderAmount().setScale(2, BigDecimal.ROUND_HALF_UP)));// 小数点后两位
			sParaTemp.put("body", "");
			// 获取请求页面数据
			...
			scanPayResultVo
					.setProductName(rpTradePaymentOrder.getProductName());
			scanPayResultVo
					.setOrderAmount(rpTradePaymentOrder.getOrderAmount());
		} else if (PayWayEnum.TEST_PAY_HTTP_CLIENT.name().equals(payWayCode)) {
			// 测试模拟httpclient支付
	 ...
	 */
	@Override
	public OrderPayResultVo completeScanPayByResult(String payWayCode,
		...
		if (verify_result) {
			if (trade_status.equals("TRADE_FINISHED")
				...
				String resultUrl = getMerchantNotifyUrl(rpTradePaymentRecord,
						rpTradePaymentOrder,
						rpTradePaymentRecord.getReturnUrl(),
	...
	@Override
	public OrderPayResultVo completeTestPay(String payKey, String productName,
			String orderNo, Date orderDate, Date orderTime,
			BigDecimal orderPrice, String payWayCode, String orderIp,
			Integer orderPeriod, String returnUrl, String notifyUrl,
			String remark, String field1, String field2, String field3,
		...
		// 根据支付产品及支付方式获取费率
		RpPayWay payWay = rpPayWayService.getByPayWayTypeCode(
				rpUserPayConfig.getProductCode(), payWayCode,
				PayTypeEnum.TEST_PAY.name());
		if (payWay == null) {
			throw new UserBizException(UserBizException.USER_PAY_CONFIG_ERRPR,
					"用户支付配置有误");
		}
		...
		orderPayResultVo.setProductName(rpTradePaymentRecord.getProductName());// 产品名称
		String resultUrl = getMerchantNotifyUrl(rpTradePaymentRecord,
				rpTradePaymentOrder, rpTradePaymentRecord.getReturnUrl(),
				TradeStatusEnum.SUCCESS);
		orderPayResultVo.setReturnUrl(resultUrl);
		orderPayResultVo.setStatus(TradeStatusEnum.SUCCESS.name());
		return orderPayResultVo;
	 ...
	 * @return
	 */
	private RpTransactionMessage sealRpTransactionMessage(
			RpTradePaymentRecord rpTradePaymentRecord) {
		// 封装会计需要的实体
		...
		rpAccountingVoucher.setBankChangeAmount(rpTradePaymentRecord
				.getOrderAmount().doubleValue());// 银行订单金额
		rpAccountingVoucher.setReceiverChangeAmount(rpTradePaymentRecord
				.getOrderAmount().doubleValue());
		rpAccountingVoucher.setReceiverAccountNo(rpTradePaymentRecord
				.getMerchantNo());
		rpAccountingVoucher.setBankAccount("");
		rpAccountingVoucher.setBankChannelCode(rpTradePaymentRecord
				.getPayWayCode());
		rpAccountingVoucher.setProfit(rpTradePaymentRecord.getPlatProfit()
				.doubleValue());
		rpAccountingVoucher.setIncome(rpTradePaymentRecord.getPlatIncome()
				.doubleValue());
		rpAccountingVoucher.setCost(rpTradePaymentRecord.getPlatCost()
				.doubleValue());
		rpAccountingVoucher.setBankOrderNo(rpTradePaymentRecord
				.getBankOrderNo());
		rpAccountingVoucher.setPayAmount(0d);
		rpAccountingVoucher.setReceiverAccountType(1);
		rpAccountingVoucher.setReceiverFee(rpTradePaymentRecord
				.getReceiverFee() == null ? 0d : rpTradePaymentRecord
				.getReceiverFee().doubleValue());
		...
		RpTransactionMessage rpTransactionMessage = new RpTransactionMessage(
				...
				NotifyDestinationNameEnum.ACCOUNTING_NOTIFY.name());
	 ...
	 * @return
	 */
	private RpTradePaymentOrder sealRpTradePaymentOrder(String merchantNo,
			String merchantName, String productName, String orderNo,
			Date orderDate, Date orderTime, BigDecimal orderPrice,
			String payWay, String payWayName, String fundIntoType,
			String orderIp, Integer orderPeriod, String returnUrl,
			String notifyUrl, String remark, String field1, String field2,
		...
		if (PayWayEnum.WEIXIN.name().equals(payWay)) {
			...
			rpTradePaymentOrder.setPayTypeCode(PayTypeEnum.TEST_PAY_HTTP_CLIENT
					.name());// 支付类型
			rpTradePaymentOrder.setPayTypeName(PayTypeEnum.TEST_PAY_HTTP_CLIENT
					.getDesc());// 支付方式
	 ...
	 * @return
	 */
	private RpTradePaymentRecord sealRpTradePaymentRecord(String merchantNo,
			String merchantName, String productName, String orderNo,
			BigDecimal orderPrice, String payWay, String payWayName,
			String fundIntoType, BigDecimal feeRate, String orderIp,
			String returnUrl, String notifyUrl, String remark, String field1,
			String field2, String field3, String field4, String field5) {
		...
		if (PayWayEnum.WEIXIN.name().equals(payWay)) {
			...
			rpTradePaymentRecord.setPayTypeName(PayTypeEnum.DIRECT_PAY
					.getDesc());// 支付方式
		} else if (PayWayEnum.TEST_PAY_HTTP_CLIENT.name().equals(payWay)) {// 模拟httpclient支付
			rpTradePaymentRecord
					.setPayTypeCode(PayTypeEnum.TEST_PAY_HTTP_CLIENT.name());// 支付类型
			rpTradePaymentRecord
					.setPayTypeName(PayTypeEnum.TEST_PAY_HTTP_CLIENT.getDesc());// 支付方式
		...
		if (FundInfoTypeEnum.PLAT_RECEIVES.name().equals(fundIntoType)) {// 平台收款
			...
			BigDecimal platIncome = orderAmount.multiply(feeRate).divide(
					BigDecimal.valueOf(100), 2, BigDecimal.ROUND_HALF_UP); // 平台收入
																			// =
																			// 订单金额
																			// *
																			// 支付费率(设置的费率除以100为真实费率)
			BigDecimal platCost = orderAmount.multiply(
					BigDecimal.valueOf(Double.valueOf(WeixinConfigUtil
							.readConfig("pay_rate")))).divide(
					BigDecimal.valueOf(100), 2, BigDecimal.ROUND_HALF_UP);// 平台成本
																			// =
																			// 订单金额
																			// *
	 ...
	 * @return
	 */
	private WeiXinPrePay sealWeixinPerPay(String appId, String mchId,
			String productName, String remark, String bankOrderNo,
			BigDecimal orderPrice, Date orderTime, Integer orderPeriod,
			WeiXinTradeTypeEnum weiXinTradeTypeEnum, String productId,
			String openId, String orderIp) {
		...
		Integer totalFee = orderPrice.multiply(BigDecimal.valueOf(100d))
				.intValue();
		weiXinPrePay.setTotalFee(totalFee);// 订单金额
		weiXinPrePay.setTimeStart(DateUtils.formatDate(orderTime,
				"yyyyMMddHHmmss"));// 订单开始时间
		weiXinPrePay.setTimeExpire(DateUtils.formatDate(
				DateUtils.addMinute(orderTime, orderPeriod), "yyyyMMddHHmmss"));// 订单到期时间
	...
	public void verifyNotify(String payWayCode, Map<String, String> notifyMap) {
		...
		if (FundInfoTypeEnum.MERCHANT_RECEIVES.name().equals(fundIntoType)) {// 商户收款
			// 根据资金流向获取配置信息
			RpUserPayInfo rpUserPayInfo = rpUserPayInfoService.getByUserNo(
					merchantNo, PayWayEnum.WEIXIN.name());
			partnerKey = rpUserPayInfo.getPartnerKey();
		...
		if (PayWayEnum.WEIXIN.name().equals(payWayCode)) {
			...
			if (WeiXinPayUtils.notifySign(notifyMap, sign, partnerKey)) {// 根据配置信息验证签名
				...
				throw new TradeBizException(
						TradeBizException.TRADE_WEIXIN_ERROR, "微信签名失败");
			}
		} else if (PayWayEnum.ALIPAY.name().equals(payWayCode)) {
			if (AlipayNotify.verify(notifyMap)) {// 验证成功
			} else {// 验证失败
				throw new TradeBizException(
						TradeBizException.TRADE_ALIPAY_ERROR, "支付宝签名异常");
			}
		} else {
			throw new TradeBizException(TradeBizException.TRADE_PAY_WAY_ERROR,
					"错误的支付方式");
	...
	@Override
	public String getMerchantNotifyMessage(String payWayCode,
			Map<String, String> resultMap) {
		...
		if (PayWayEnum.WEIXIN.name().equals(payWayCode)) {
			if (WeixinTradeStateEnum.SUCCESS.name().equals(
					resultMap.get("result_code"))) {// 业务结果 成功
				resultUrl = getMerchantNotifyUrl(rpTradePaymentRecord,
						rpTradePaymentOrder,
						rpTradePaymentRecord.getReturnUrl(),
						TradeStatusEnum.SUCCESS);
			} else {
		...
		return getNotifySendStr(resultUrl,
				rpTradePaymentRecord.getMerchantOrderNo(),
				rpTradePaymentRecord.getMerchantNo());
	}
	 ...
	 *            商户编号
	 */
	public String getNotifySendStr(String notifyUrl, String merchantOrderNo,
			String merchantNo) {
