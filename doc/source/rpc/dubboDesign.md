...
## 整体设计
![dubbo-framework-min](https://s1.wailian.download/2020/01/17/dubbo-framework-min.jpg)

图例说明：
- 左边淡蓝背景的为服务消费方使用的接口，右边淡绿色背景的为服务提供方使用的接口，位于中轴线上的为双方都用到的接口。
- 从下至上分为十层，各层均为单向依赖，右边的黑色箭头代表层之间的依赖关系，每一层都可以剥离上层被复用，其中，Service 和 Config 层为 API，其它各层均为 SPI。
- 绿色小块的为扩展接口，蓝色小块为实现类，只显示用于关联各层的实现类。
...
- 不用 Client 和 Server 的原因是 Dubbo 在很多场景下都使用 Provider, Consumer, Registry, Monitor 划分逻辑拓普节点，保持统一概念。
...
## 模块分包
![dubbo-modules-min](https://s1.wailian.download/2020/01/17/dubbo-modules-min.jpg)
...
## 依赖关系
![dubbo-relation-min](https://s1.wailian.download/2020/01/17/dubbo-relation-min.jpg)

图例说明：
- 小方块代表层或模块，蓝色的表示与业务有交互，绿色的表示只对 Dubbo 内部交互。
- 背景方块 Consumer, Provider, Registry, Monitor 代表部署逻辑拓扑节点。
- 图中只包含 RPC 的层，不包含 Remoting 的层，Remoting 整体都隐含在 Protocol 中。

## 调用链
展开总设计图的红色调用链，如下：

![dubbo-extension-min](https://s1.wailian.download/2020/01/17/dubbo-extension-min.jpg)

## 暴露服务时序
![dubbo-export-min](https://s1.wailian.download/2020/01/17/dubbo-export-min.jpg)

## 引用服务时序
![dubbo-refer-min](https://s1.wailian.download/2020/01/17/dubbo-refer-min.jpg)
