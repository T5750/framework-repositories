# MySQL

## Contents
- [MySQL Sample Database](doc/mysqlSampleDatabase.md)
- [MySQL Index](doc/mysqlIndex.md)
    - [Creating indexes](doc/mysqlIndex.md#Creating-indexes)
    - [Removing indexes](doc/mysqlIndex.md#Removing-indexes)
    - [Listing table indexes](doc/mysqlIndex.md#Listing-table-indexes)
    - [Unique indexes](doc/mysqlIndex.md#Unique-indexes)
    - [Prefix indexes](doc/mysqlIndex.md#Prefix-indexes)
    - [Invisible indexes](doc/mysqlIndex.md#Invisible-indexes)
    - [Descending indexes](doc/mysqlIndex.md#Descending-indexes)
    - [Composite indexes](doc/mysqlIndex.md#Composite-indexes)
    - [Clustered indexes](doc/mysqlIndex.md#Clustered-indexes)
    - [Index cardinality](doc/mysqlIndex.md#Index-cardinality)
    - [USE INDEX hint](doc/mysqlIndex.md#USE-INDEX-hint)
    - [FORCE INDEX hint](doc/mysqlIndex.md#FORCE-INDEX-hint)
- [MySQL Functions](doc/mysqlFunction.md)
    - [Aggregate Functions](doc/mysqlFunction.md)
    - [Control Flow Functions](doc/mysqlFunction.md#control-flow-functions)
    - [Comparison Functions](doc/mysqlFunction.md#comparison-functions)
    - [Date Functions](doc/mysqlFunctionDate.md)
    - [String Functions](doc/mysqlFunctionString.md)
    - [Math Functions](doc/mysqlFunctionMath.md)
    - [Window Functions](doc/mysqlFunctionWindow.md)
- [Grafana](doc/Grafana.md)

## Runtime Environment
- [MySQL 5.6](http://www.mysql.com/)