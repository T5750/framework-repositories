...
public class RedisTransactionDao implements TransactionDao {
	...
	@Override
	public List<TransactionVo> findTransactions(final String domain,
			final Integer pageNum, final int pageSize) {
		return RedisHelper.execute(jedisPool,
				new JedisCallback<List<TransactionVo>>() {
					@Override
					public List<TransactionVo> doInJedis(Jedis jedis) {
						int start = (pageNum - 1) * pageSize;
						int end = pageNum * pageSize;
						ArrayList<byte[]> allKeys = new ArrayList<byte[]>(
								jedis.keys((domainKeyPrefix.getProperty(domain) + "*")
										.getBytes()));
						...
						for (byte[] key : keys) {
							byte[] content = RedisHelper
									.getKeyValue(jedis, key);
							Map<String, Object> map = (Map<String, Object>) serializer
									.deserialize(content);
							TransactionVo transactionVo = new TransactionVo();
							transactionVo.setDomain(domain);
							transactionVo.setGlobalTxId(DatatypeConverter
									.printHexBinary((byte[]) map
											.get("GLOBAL_TX_ID")));
							transactionVo.setBranchQualifier(DatatypeConverter
									.printHexBinary((byte[]) map
											.get("BRANCH_QUALIFIER")));
							transactionVo.setStatus((Integer) map.get("STATUS"));
							transactionVo.setTransactionType((Integer) map
									.get("TRANSACTION_TYPE"));
							transactionVo.setRetriedCount((Integer) map
									.get("RETRIED_COUNT"));
							transactionVo.setCreateTime((Date) map
									.get("CREATE_TIME"));
							transactionVo.setLastUpdateTime((Date) map
	...
	public Integer countOfFindTransactions(String domain) {
		...
		try {
			...
			return jedis.keys(
					(domainKeyPrefix.getProperty(domain) + "*").getBytes())
					.size();
		} catch (Exception e) {
	...
	@Override
	public boolean resetRetryCount(final String domain,
			final byte[] globalTxId, final byte[] branchQualifier) {
		...
		return RedisHelper.execute(jedisPool, new JedisCallback<Boolean>() {
			...
			public Boolean doInJedis(Jedis jedis) {
				byte[] key = RedisHelper.getRedisKey(
						domainKeyPrefix.getProperty(domain),
						new TransactionXid(globalTxId, branchQualifier));
				...
				Long result = jedis.hsetnx(key,
						ByteUtils.longToBytes((Long) map.get("VERSION")),
